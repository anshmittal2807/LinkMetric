...
function HeroSection() {
  ...
  const handleShorten = async () => {
    try {
      ...
      if (!user) {
        throw new Error("Please login first to shorten links")
      }

      setShortening(true)

      linkValue.trim() === '' &&
        (() => { throw new Error('URL can not be blank') })()
  ...
  return (
    <section className="relative mx-auto flex max-w-7xl flex-col items-center px-6 pb-12 pt-14 text-center lg:px-8 lg:pb-16 lg:pt-20">
      ...
      <div className="mt-10 w-full max-w-4xl rounded-3xl border border-[#c3c6d7] bg-white p-3 shadow-[0_24px_60px_-34_rgba(15,23,42,0.55)] backdrop-blur-md">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-stretch">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#004ac6] px-7 py-4 text-base font-semibold text-white shadow-[0_18px_40px_-24px_rgba(0,74,198,0.9)] transition hover:bg-[#003ea8] active:scale-[0.99]"
          >
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3 text-xs font-semibold uppercase tracking-[0.2em] text-[#737686]">
        ...
        <span>
          Used by fast-moving teams for
        </span>


        <span className="rounded-full border border-[#d3e4fe] bg-white px-3 py-1 text-[#004ac6]">
          Campaigns
        </span>
