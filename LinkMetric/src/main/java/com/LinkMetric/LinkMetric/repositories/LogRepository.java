...
import com.LinkMetric.LinkMetric.Dtos.analytics.WeeklyClickCount;
...
public interface LogRepository extends JpaRepository<Log , Long> {
    ...
    @Query(value = """
        SELECT
            YEARWEEK(click_time_and_date) as week,
            COUNT(*) as clicks
        FROM log
        WHERE link_link_id = :linkId
        GROUP BY YEARWEEK(click_time_and_date)
        ORDER BY week
        LIMIT 7
        """, nativeQuery = true)
    List<WeeklyClickCount> getWeeklyClicks(Long linkId);
